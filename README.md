...
直接回车使用 `config.ini` 中 `show_on_open` 的默认值。
...
- 进入导图时会展开并聚焦根节点，然后自动执行“缩放至全屏”，展示第一层分支概览。
- 双击工具栏主页按钮可以随时重新显示封面。
...
默认配置文件是 `config.ini`。图片、Logo、标题等可选文本值可以留空；也可以在行首使用 `;` 注释配置行。布尔值和颜色等字段应保留合法格式。
...
```bash
python3 md_to_mindmap.py "大纲.md" --config "configs/dark.ini"
```
